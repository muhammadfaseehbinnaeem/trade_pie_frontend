...
const ReferralCommissionsScreen = ({ navigation }) => {
    ...
    const referralCommissionsNotFoundAlertOkHandler = () => {
        status === 'All' ? navigation.navigate('AdminDashboard') : navigation.navigate('UserReferralCommissions');
        setStatus('All');
        setIsLoading(false);
    };
            ...
            const getUserReferralCommissions = async () => {
                ...
                try {
                    const response = await sendRequest(
                        `/api/commissions/allreferral/${status}`,
                        'GET',
                        null,
                    ...
                    if (responseData?.success) {
                        ...
                        if (referralCommissionsData.length === 0) {
                            return (
                                Alert.alert(
                                    'Message',
                                    'No referral commissions found in this category.',
                                    [{
                                        text: 'OK',
...
export default ReferralCommissionsScreen;